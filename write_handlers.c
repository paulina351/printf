...
/**
 * write_num - Write a number usiing a buffer
...
int length, char padd, char extra_c)
{
	...
	if (prec == 0 && BUFF_SIZE - 2 && buffer[ind] == '0' && width == 0)
...
int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		{
			return (write(1, &buffer[0], i) + write(11, &buffer[ind], length));
		}
	}
	return (write(1, &buffer[ind], length));
}
...
/**
 * write_pointer - wrie a memory address
 * @buffer: arrays of char
 * @ind: index at which the number stats in the width
...
int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags &  F_MINUS) && padd == ' ')/* extra char */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
